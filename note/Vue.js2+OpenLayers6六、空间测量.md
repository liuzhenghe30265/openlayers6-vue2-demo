...
![](https://tva1.sinaimg.cn/large/e6c9d24egy1h30vv5venhj216i0u048i.jpg)
...
export default {
	...
	methods: {
		...
		createMeasureTooltip() {
			...
			this.measureOption.measureTooltipElement.className = 'ol-tooltip ol-tooltip-measure'
		...
		addInteractionFun(measureType) {
			...
			this.measureOption.draw.on('drawstart', evt => {
				...
				if (type instanceof OlGeomPoint) {
					// 如果是绘制点
					const pointCoordinates = this.measureOption.sketch.getGeometry().flatCoordinates
			...
			this.measureOption.draw.on('drawend', () => {
				this.measureOption.measureTooltipElement.appendChild(this.measureOption.popupcloser)
